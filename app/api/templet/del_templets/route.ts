  ...
  try {
    ...
    if (!selected || !Array.isArray(selected)) {
      return NextResponse.json(
        { success: false, msg: 'Selected templates are required' },
        { status: 400 }
      )
    }
    ...
    const apiUrl = `${backendUrl}/api/templet/del_templets`
    ...
    console.error('Error in del_templets API:', error)
    return NextResponse.json(
      { 
        success: false, 
        msg: "Failed to delete templates",
