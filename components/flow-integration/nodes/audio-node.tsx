...
import { Upload, X, Plus, Save, Trash2, Edit, Star, Link2, Volume2 } from "lucide-react"
...
const buildNodeData = (
  ...
  type: "audioMessage" as const,
  data: {
    state: {
      messageType: "audio",
      contentType: "audio",
      audioUrl: audioUrl || "",
      audioCaption: captions || ""
    }
  },
  title,
  messageNumber,
...
export function AudioNode({ data, selected, id }: any) {
  ...
  const [title, setTitle] = useState("Audio Message")
  ...
  useEffect(() => {
    ...
    setUploadedFile(data?.audioFile || null)
    setAudioUrl(data?.audioUrl || "")
    setTitle(data?.title || "Audio Message")
    setMessageNumber(data?.messageNumber || 1)
    setOptions(initializeOptions(data?.options))
  }, [data])

  const syncData = useCallback((customData = {}) => {
    const newData = buildNodeData(audioUrl, captions, title, messageNumber, options, uploadedFile)
    updateNode?.(id, { ...newData, ...customData })
  }, [audioUrl, captions, title, messageNumber, options, uploadedFile, updateNode, id])
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith("audio/")) {
      toast({ title: "Error", description: "Invalid audio file", variant: "destructive" })
      return
    }
    ...
    setTimeout(() => {
      ...
      updateNode?.(id, { ...newData, audioFile: file })
    }, 0)
    
    toast({ title: "Success", description: "Audio uploaded", variant: "default" })
  }
  ...
  const addOption = () => {
    ...
    setTimeout(() => {
      const newData = buildNodeData(audioUrl, captions, title, messageNumber, newOptions, uploadedFile)
      ...
      <div className={`w-[320px] bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm ${selected ? "ring-2 ring-blue-500" : ""}`}>
        ...
        <div className="bg-pink-500 text-white px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleTitleEdit} className="p-1 hover:bg-pink-600 rounded transition-colors" title="Edit title">
              <Edit className="w-3 h-3" />
            </button>
          </div>
          <div className="flex items-center gap-1">
            <button onClick={handleSetStartNode} className="p-1 hover:bg-pink-600 rounded transition-colors" title="Set Start">
        ...
        <div className="p-4">
          <div className="flex justify-center gap-4 mb-2">
            <button onClick={() => setUseUrlInput(false)} className={`text-sm px-3 py-1 rounded ${!useUrlInput ? "bg-pink-500 text-white" : "bg-gray-200"}`}>
              Upload File
            </button>
            <button onClick={() => setUseUrlInput(true)} className={`text-sm px-3 py-1 rounded ${useUrlInput ? "bg-pink-500 text-white" : "bg-gray-200"}`}>
              Paste URL
            </button>
          </div>
          ...
          {!useUrlInput ? (
            <div className="bg-gray-100 border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
              <input type="file" accept="audio/*" onChange={handleFileUpload} className="hidden" id={`audio-upload-${id}`} />
