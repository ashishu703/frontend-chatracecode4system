...
export function ImageNode({ data, selected, id }: any) {
  ...
  const [title, setTitle] = useState("Image Message")
  ...
  useEffect(() => {
    ...
    setTitle(data?.title || "Image Message")
  ...
  const addOption = () => {
    ...
    setTimeout(() => {
      const newData = buildNodeData(imageUrl, captions, title, messageNumber, newOptions, uploadedFile)
      ...
      <div className={`w-[320px] bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm ${selected ? "ring-2 ring-blue-500" : ""}`}>
        ...
        <div className="p-4">
          <div className="flex justify-center gap-4 mb-2">
            <button onClick={() => setUseUrlInput(false)} className={`text-sm px-3 py-1 rounded ${!useUrlInput ? "bg-purple-500 text-white" : "bg-gray-200"}`}>
              Upload File
            </button>
            <button onClick={() => setUseUrlInput(true)} className={`text-sm px-3 py-1 rounded ${useUrlInput ? "bg-purple-500 text-white" : "bg-gray-200"}`}>
              Paste URL
            </button>
          </div>
