...
import { FileText, X, Upload, Plus, Save, Trash2, Edit, Star, Link2 } from "lucide-react"
...
const buildNodeData = (
  ...
  type: "documentMessage" as const,
  data: {
    state: {
      messageType: "document",
      contentType: "document",
      documentUrl: documentUrl || "",
      documentCaption: captions || ""
    }
  },
  title,
  messageNumber,
...
export function DocumentMessage({ data, selected, id }: any) {
  ...
  const [title, setTitle] = useState("Document Message")
  ...
  useEffect(() => {
    ...
    setUploadedFile(data?.documentFile || null)
    setDocumentUrl(data?.documentUrl || "")
    setTitle(data?.title || "Document Message")
    setMessageNumber(data?.messageNumber || 1)
    setOptions(initializeOptions(data?.options))
  }, [data])

  const syncData = useCallback((customData = {}) => {
    const newData = buildNodeData(documentUrl, captions, title, messageNumber, options, uploadedFile)
    updateNode?.(id, { ...newData, ...customData })
  }, [documentUrl, captions, title, messageNumber, options, uploadedFile, updateNode, id])
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowedTypes.includes(fileExtension)) {
      toast({ title: "Error", description: "Invalid file type. Allowed: PDF, DOC, DOCX, TXT, RTF, ODT", variant: "destructive" })
      return
    }
    ...
    setTimeout(() => {
      ...
      updateNode?.(id, { ...newData, documentFile: file })
    }, 0)
    
    toast({ title: "Success", description: "Document uploaded", variant: "default" })
  }
  ...
  const addOption = () => {
    ...
    setTimeout(() => {
      const newData = buildNodeData(documentUrl, captions, title, messageNumber, newOptions, uploadedFile)
      ...
      <div className={`w-[320px] bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm ${selected ? "ring-2 ring-blue-500" : ""}`}>
        ...
        <div className="bg-teal-500 text-white px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleTitleEdit} className="p-1 hover:bg-teal-600 rounded transition-colors" title="Edit title">
              <Edit className="w-3 h-3" />
            </button>
          </div>
          <div className="flex items-center gap-1">
            <button onClick={handleSetStartNode} className="p-1 hover:bg-teal-600 rounded transition-colors" title="Set Start">
        ...
        <div className="p-4">
          <div className="flex justify-center gap-4 mb-2">
            <button onClick={() => setUseUrlInput(false)} className={`text-sm px-3 py-1 rounded ${!useUrlInput ? "bg-teal-500 text-white" : "bg-gray-200"}`}>
              Upload File
            </button>
            <button onClick={() => setUseUrlInput(true)} className={`text-sm px-3 py-1 rounded ${useUrlInput ? "bg-teal-500 text-white" : "bg-gray-200"}`}>
              Paste URL
            </button>
          </div>
          ...
          {!useUrlInput ? (
            <div className="bg-gray-100 border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
              <input type="file" accept=".pdf,.doc,.docx,.txt,.rtf,.odt" onChange={handleFileUpload} className="hidden" id={`document-upload-${id}`} />
              <label htmlFor={`document-upload-${id}`} className="cursor-pointer">
