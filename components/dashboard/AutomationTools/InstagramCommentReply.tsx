...
import { instagramCommentAPI, CommentAutomationSettings } from '@/utils/api/comment-automation/comment-automation-api';
...
const CommentSettingsUI: React.FC<CommentSettingsUIProps> = ({ onBack }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await instagramCommentAPI.saveSettings(settings);
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const response = await instagramCommentAPI.deleteSettings();
      if (response.success) {
        toast({
          title: "Success",
    ...
    <div className="w-full max-w-full bg-gray-50 font-sans p-6 pb-8 min-h-screen">
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          ...
          <h1 className="text-2xl font-bold text-gray-900 flex-1 text-center">Instagram Comment Settings</h1>
            ...
            {trackComments === 'specific' && (
              <div className="mt-4">
                ...
                >
                  ...
                  <option value="post6">Post 6 - Instagram story highlight</option>
                  <option value="post7">Post 7 - Reel showcase</option>
                </select>
              </div>
            )}
          </CardContent>
